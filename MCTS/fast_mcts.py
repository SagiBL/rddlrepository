...
class MCTS:
    ...
    def step(self):   # preforms one step of expansion and simulates it
        ...
        if node.state['signal-t___i0'] == max_green: #if reached the maximum time without changing
            tmp_state, one_step = self.red_sequence(node.state)
            node.child_change = Node(node, tmp_state)  # create the change_child
            node.child_change.total_reward = self.simulate(one_step)
            self.root_node = self.back_propagate(node, node.child_change.total_reward)
            ...
            self.root_node = self.back_propagate(node, node.child_stay.total_reward)
            tmp_state, one_step = self.red_sequence(node.state)
            node.child_change = Node(node, tmp_state)  # create the change_child
            node.child_change.total_reward = self.simulate(one_step)
            self.root_node = self.back_propagate(node, node.child_change.total_reward)
    ...
    def red_sequence(self, state):
        tmp_env = pyRDDLGym.make('TrafficBLX_SimplePhases', 0)
        _ = tmp_env.reset()
        tmp_env.set_state(state)
        end_state = state
        one_step = state
        for red_step in range(red_time):
            end_state, _, terminated, truncated, _ = tmp_env.step(self.stay)
            if red_step == 0:
                one_step = end_state
            if truncated or terminated:
                break

        end_state, _, _, _, _ = tmp_env.step(self.change)

        for green_step in range(min_green-1):
            end_state, _, terminated, truncated, _ = tmp_env.step(self.stay)
            if truncated or terminated:
                break

        tmp_env.close()
        return end_state, one_step
